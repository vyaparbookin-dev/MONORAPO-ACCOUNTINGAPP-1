...
-- ============================================================================
-- 17. MULTI-BRANCH & FRANCHISE ENGINE (ENTERPRISE CORE)
-- ============================================================================
...
-- ============================================================================
-- 18. HARDWARE ABSTRACTION LAYER (HAL) CONTROLLER PROFILES
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.machine_hardware_profiles (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
    profile_name VARCHAR(100) NOT NULL,
    reader_protocol VARCHAR(50) DEFAULT 'WIFI_MQTT_PULSE', -- 'WIFI_MQTT_PULSE', 'RS485_MODBUS', 'USB_SERIAL'
    pulse_duration_ms INT DEFAULT 50,
    signal_voltage_v NUMERIC(4, 1) DEFAULT 12.0,
    pulses_per_credit INT DEFAULT 1,
    credit_timeout_sec INT DEFAULT 30,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================================================
-- 19. ENTERPRISE MULTI-TIER SMARTCARDS & AUDIT LEDGER
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.gamezone_playcards_v2 (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
    ...
    -- Multi-Tier Card Types
    card_type VARCHAR(50) DEFAULT 'pay_per_play', -- 'pay_per_play', 'time_based', 'points_based', 'family_group'
    membership_tier VARCHAR(50) DEFAULT 'STANDARD', -- 'STANDARD', 'SILVER', 'GOLD', 'PLATINUM_VIP'
    
    -- Balance Ledgers
    real_cash_balance NUMERIC(10, 2) DEFAULT 0.00,
    bonus_cash_balance NUMERIC(10, 2) DEFAULT 0.00,
    digital_tickets INT DEFAULT 0,
    loyalty_points INT DEFAULT 0,
    
    -- Time-Based Unlimited Pass Rules
    time_pass_expires_at TIMESTAMPTZ,
    is_time_pass_active BOOLEAN DEFAULT FALSE,
    
    -- Family/Group Linked Wristbands
    parent_card_id UUID REFERENCES public.gamezone_playcards_v2(id) ON DELETE SET NULL,
    linked_wristbands JSONB DEFAULT '[]'::jsonb,
    
    -- Card Lifecycle & Security
    card_status VARCHAR(50) DEFAULT 'ACTIVE', -- 'ACTIVE', 'LOCKED', 'MIGRATED_LOST', 'EXPIRED'
    last_tapped_machine VARCHAR(150),
    last_tapped_at TIMESTAMPTZ,
...
-- Lost Card Migration Audit Trail
CREATE TABLE IF NOT EXISTS public.card_migrations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
    old_card_uid VARCHAR(100) NOT NULL,
    new_card_uid VARCHAR(100) NOT NULL,
    migrated_real_cash NUMERIC(10, 2) DEFAULT 0,
    migrated_bonus_cash NUMERIC(10, 2) DEFAULT 0,
    migrated_tickets INT DEFAULT 0,
    operator_staff_name VARCHAR(150),
    reason TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Immutable Financial Ledger
CREATE TABLE IF NOT EXISTS public.playcard_ledger (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
    branch_id UUID REFERENCES public.branches(id) ON DELETE SET NULL,
    card_uid VARCHAR(100) NOT NULL,
    transaction_type VARCHAR(50) NOT NULL, -- 'RECHARGE', 'MACHINE_PLAY', 'TIME_PASS_USE', 'TICKET_WON', 'PRIZE_REDEEM', 'MIGRATION'
    amount NUMERIC(10, 2) DEFAULT 0.00,
    bonus_amount NUMERIC(10, 2) DEFAULT 0.00,
    tickets INT DEFAULT 0,
    balance_after NUMERIC(10, 2) NOT NULL,
    device_or_machine_id VARCHAR(100),
