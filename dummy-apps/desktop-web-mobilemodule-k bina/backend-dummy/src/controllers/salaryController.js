...
export const addSalary = async (req, res) => {
  try {
    ...
    const salaryData = { ...normalizeSalaryPayload(req.body), companyId: req.companyId };
...
export const getSalaryById = async (req, res) => {
  try {
    const salary = await Salary.findOne({ _id: req.params.id, companyId: req.companyId, isDeleted: false });
...
export const updateSalary = async (req, res) => {
  try {
    const salary = await Salary.findOneAndUpdate(
      ...
      { $set: { ...normalizeSalaryPayload(req.body), companyId: req.companyId } },
      { new: true }
    );
    if (!salary) return res.status(404).json({ success: false, error: "Salary record not found" });
    res.json({ success: true, salary });
