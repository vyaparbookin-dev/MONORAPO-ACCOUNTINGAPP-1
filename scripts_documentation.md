...
## `scripts/find-usage.js` and `scripts/find-service-usage.js`

**Purpose:**

These scripts are used to find where specific files are being imported and used within the applications. This is particularly useful for identifying whether a file is still in use before deleting it, helping to prevent dead code.

*   `find-usage.js`: A more generic and configurable version.
*   `find-service-usage.js`: An older, more specific version focused on the `mobile` app.

**How to Use:**

You can configure the `files` array within the script to specify which files you want to search for.
