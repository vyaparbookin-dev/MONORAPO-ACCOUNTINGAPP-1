...
export default function StaffPerformancePage() {
  ...
  const [kpis, setKpis] = useState({ totalStaff: 0, totalRevenue: 0, topPerformer: 'N/A', totalCovers: 0, totalUpsells: 0 });
  ...
  const fetchPerformanceData = async () => {
    try {
      ...
      const performanceData = res.data && Array.isArray(res.data) ? res.data : [];
      ...
      const totalCovers = performanceData.reduce((sum, s) => sum + (s.coversServed || 0), 0);
      const totalUpsells = performanceData.reduce((sum, s) => sum + (s.upsellingCount || 0), 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="h-72 flex items-center justify-center">
            {staffData.length === 0 ? (
              <p className="text-slate-400 text-xs font-semibold">कोई स्टाफ रेवेन्यू डेटा उपलब्ध नहीं है</p>
            ) : (
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="h-60 flex items-center justify-center">
            {staffData.length === 0 ? (
              <p className="text-slate-400 text-xs font-semibold">कोई बिल शेयर डेटा उपलब्ध नहीं है</p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
              ...
              {staffData.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-8 text-slate-400 font-medium">
                    कोई स्टाफ परफॉरमेंस डेटा उपलब्ध नहीं है
                  </td>
                </tr>
              ) : (
                staffData.map((staff, idx) => (
                  <tr key={idx} className="border-t border-slate-100 hover:bg-amber-50/40 transition">
                    ...
                    <td className="p-3.5 text-slate-600 font-medium">{staff.role || "Staff"}</td>
                    <td className="p-3.5 text-center font-bold text-slate-800">{staff.bills || 0}</td>
                    <td className="p-3.5 text-center font-mono font-bold text-purple-700">👤 {staff.coversServed || 0}</td>
                    <td className="p-3.5 text-center">
                      <span className="px-2 py-0.5 bg-pink-100 text-pink-800 font-black rounded-lg border border-pink-200">
                        🧁 {staff.upsellingCount || 0} Items
                      </span>
                    </td>
                    <td className="p-3.5 text-right font-black text-emerald-700 font-mono">₹{(staff.revenue || 0).toLocaleString()}</td>
