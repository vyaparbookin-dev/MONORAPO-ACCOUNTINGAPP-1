...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    try {
      // Auto-heal any existing string IDs saved mistakenly in previous imports
      await Category.updateMany({ companyId: companyId }, { $set: { companyId: companyObjectId } });
      await SubCategory.updateMany({ companyId: companyId }, { $set: { companyId: companyObjectId } });
      await Brand.updateMany({ companyId: companyId }, { $set: { companyId: companyObjectId } });
      await Unit.updateMany({ companyId: companyId }, { $set: { companyId: companyObjectId } });
    ...
    for (const item of validProducts) {
      ...
      if (isAutoGeneratedBarcode) {
        ...
        while (seenBarcodes.has(newBarcode.toLowerCase()) || existingBarcodeSet.has(newBarcode.toLowerCase())) {
          newBarcode = generateBarcode(item.sku);
        }
