...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    console.log("INSERT OPS:", insertDocs.length);
    console.log("UPDATE OPS:", updateOps.length);

    let finalInserted = 0, finalUpdated = 0, finalMatched = 0, finalUpserted = 0;

    // 1. Process Updates
    if (updateOps.length > 0) {
      console.log(`🚀 [DEBUG BULK IMPORT] Executing DB bulkWrite for ${updateOps.length} updates...`);
      const updateResult = await Product.bulkWrite(updateOps, { ordered: false });
      if (updateResult) {
        finalUpdated = updateResult.modifiedCount || updateResult.nModified || updateResult.result?.nModified || 0;
        finalMatched = updateResult.matchedCount || updateResult.nMatched || updateResult.result?.nMatched || 0;
      }
    }
