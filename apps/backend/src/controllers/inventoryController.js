...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    for (const p of formattedProducts) {
        ...
        let key = `name_${p.name.toLowerCase()}`;
        if (!isAutoSku) {
            key = `sku_${p.sku.toLowerCase()}`;
        } else if (!isAutoBarcode) {
            key = `barcode_${p.barcode.toLowerCase()}`;
        }
        uniqueMap.set(key, p);
    }
    ...
    const bulkOps = deduplicatedProducts.map(item => {
      ...
      // Smart matching: Jo unique data Excel se aaya hai wahi use karein
      let filter = { companyId: item.companyId, name: item.name };
      if (!isAutoGeneratedSku) {
          filter = { companyId: item.companyId, sku: item.sku };
      } else if (!isAutoGeneratedBarcode) {
          filter = { companyId: item.companyId, barcode: item.barcode };
      }
      ...
      return {
        updateOne: {
          ...
          update: {
            $set: updateFields,
            $setOnInsert: { sku: item.sku, barcode: item.barcode }
          },
          upsert: true
        }
      };
