...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    const bulkOps = validProducts.map(item => {
      ...
      if (matchId) {
        // Safe Update: Ensure we don't accidentally steal another product's SKU/Barcode
        if (skuMap.has(sku.toLowerCase()) && skuMap.get(sku.toLowerCase()) !== matchId) {
            sku = `SKU-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        }
        if (barcodeMap.has(barcode.toLowerCase()) && barcodeMap.get(barcode.toLowerCase()) !== matchId) {
            barcode = `BAR-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        }

        // Update the maps to prevent future collisions in this loop
        skuMap.set(sku.toLowerCase(), matchId);
        barcodeMap.set(barcode.toLowerCase(), matchId);
          ...
          updateOne: {
            ...
            update: { 
                $set: { 
                    ...
                    ...( !isAutoGeneratedBarcode && { barcode } )
                } 
            }
          }
        };
