...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    const existingProducts = await Product.find({ companyId: companyObjectId }).select('_id name sku barcode').lean();
    ...
    const bulkOps = validatedProducts.map(item => {
      // Extract internal tracking fields and any rogue _id from Excel to prevent silent drops
      const { sku, barcode, matchId, isAutoGeneratedSku, isAutoGeneratedBarcode, _id, ...updateFields } = item;

      // Force ObjectId on companyId to prevent Mongoose from silently dropping operations during cast
      const compIdObj = new mongoose.Types.ObjectId(item.companyId);
      updateFields.companyId = compIdObj;

      if (matchId) {
        return {
          updateOne: {
            // Explicitly cast _id to ObjectId for safety in bulkWrite
            filter: { _id: new mongoose.Types.ObjectId(matchId) },
            update: { 
                $set: { 
                    ...updateFields,
        ...
        // Naye product ke liye zaroori required defaults aur strict clean properties
        const newProduct = {
            ...updateFields,
            sku: item.sku,
    ...
    // === DEBUG LOGS ADDED AS PER YOUR ANALYSIS ===
    console.log("formattedProducts count:", formattedProducts.length);
    console.log("validProducts count:", validProducts.length);
    console.log("validatedProducts count:", validatedProducts.length);
    console.log("bulkOps count:", bulkOps.length);
    if (validatedProducts.length > 0) {
      console.log("FIRST VALIDATED PRODUCT", JSON.stringify(validatedProducts[0], null, 2));
    }
    if (bulkOps.length > 0) {
      console.log("FIRST BULK OP:", JSON.stringify(bulkOps[0], null, 2));
    }

    console.log("INSERT OPS", bulkOps.filter(op => op.insertOne).length);
    console.log("UPDATE OPS", bulkOps.filter(op => op.updateOne).length);
    ...
    console.log("TOTAL PRODUCTS AFTER:", totalProductsAfter);

    // Robust count extraction for ALL MongoDB driver versions
    let finalInserted = 0, finalUpdated = 0, finalMatched = 0, finalUpserted = 0;
    if (result) {
        finalInserted = result.insertedCount || result.nInserted || result.result?.nInserted || 0;
        finalUpdated = result.modifiedCount || result.nModified || result.result?.nModified || 0;
        finalMatched = result.matchedCount || result.nMatched || result.result?.nMatched || 0;
        finalUpserted = result.upsertedCount || result.nUpserted || result.result?.nUpserted || 0;
        
        // Extra logging for safety
        console.log(`🚀 [DEBUG RAW COUNTS]`, JSON.stringify({
            inserted: finalInserted, updated: finalUpdated, matched: finalMatched, rawResultKeys: Object.keys(result)
        }));
    }
    ...
    // Parse Mongoose/MongoDB BulkWrite errors directly into array
