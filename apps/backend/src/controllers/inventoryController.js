...
export const bulkImportProducts = async (req, res) => {
  try {
    ...
    for (const p of existingProducts) {
        nameMap.set(p.name.toLowerCase(), String(p._id));
        if (p.sku) skuMap.set(p.sku.toLowerCase(), String(p._id));
        if (p.barcode) barcodeMap.set(p.barcode.toLowerCase(), String(p._id));
    }

    const bulkOps = validProducts.map(item => {
      let { sku, barcode, ...updateFields } = item;
      const isAutoGeneratedSku = item.sku.startsWith('SKU-');
      const isAutoGeneratedBarcode = item.barcode.startsWith('BAR-');

      let matchId = null;
      // PRIORITY 1: OVERRIDE BY BARCODE
      if (!isAutoGeneratedBarcode && barcodeMap.has(barcode.toLowerCase())) {
          matchId = barcodeMap.get(barcode.toLowerCase());
      } 
      // PRIORITY 2: OVERRIDE BY SKU
      else if (!isAutoGeneratedSku && skuMap.has(sku.toLowerCase())) {
          matchId = skuMap.get(sku.toLowerCase());
      } 
      // PRIORITY 3: OVERRIDE BY NAME
      else if (nameMap.has(item.name.toLowerCase())) {
          matchId = nameMap.get(item.name.toLowerCase());
      }

      if (matchId) {
        const updateDoc = { ...updateFields };
        
        // SAFE UPDATE: Prevent stealing another product's unique keys
        if (!isAutoGeneratedSku) {
            const owner = skuMap.get(sku.toLowerCase());
            if (!owner || owner === matchId || owner === "new") updateDoc.sku = sku;
        }
        if (!isAutoGeneratedBarcode) {
            const owner = barcodeMap.get(barcode.toLowerCase());
            if (!owner || owner === matchId || owner === "new") updateDoc.barcode = barcode;
        }
        ...
        // Safe Insert: If SKU/Barcode already exists in DB, randomize it to prevent E11000 crash
        if (skuMap.has(sku.toLowerCase())) {
            sku = `SKU-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        }
        if (barcodeMap.has(barcode.toLowerCase())) {
            barcode = `BAR-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        }

        // Add to maps to prevent subsequent new items from claiming the same
        skuMap.set(sku.toLowerCase(), "new");
        barcodeMap.set(barcode.toLowerCase(), "new");

        return { insertOne: { document: { ...item, sku, barcode } } };
      }
    });
