...
const ReportsPage = () => {
  ...
  const [metrics, setMetrics] = useState({ totalIncome: 0, totalExpenses: 0, totalProfit: 0 });
  ...
  const fetchReports = async () => {
    try {
      ...
      // STRICT OFFLINE FIRST: Fetch ONLY from SQLite DB via dbService
      try {
        localInvoices = await dbService.getInvoices();
        localExpenses = await dbService.getExpenses();
      } catch(e) { console.error("SQLite DB error:", e); }

      let tIncome = 0;
      let tExpense = 0;
      let fetchedReports = [];

      // Determine filtering logic (Basic Month/Year logic for demo)
      const now = new Date();
      const isWithinRange = (dateStr) => {
        ...
        return true; // For week/quarter fallback
      };
      ...
      localInvoices.forEach(inv => {
        ...
        if (isWithinRange(date)) {
          const amount = Number(inv.total_amount || inv.total || inv.finalAmount || 0);
          tIncome += amount;
          fetchedReports.push({
            date: new Date(date).toLocaleDateString(),
            type: 'Income',
            description: `Invoice ${inv.invoice_number || inv.billNumber || 'Walk-in'}`,
            amount: amount,
            category: 'Sales',
      ...
      // Filter table data based on selected Report Type dropdown
      if (reportType === 'income') {
        fetchedReports = fetchedReports.filter(r => r.type === 'Income');
      } else if (reportType === 'expenses') {
        fetchedReports = fetchedReports.filter(r => r.type === 'Expense');
      }

      // Sort for table (Latest first)
      fetchedReports.sort((a, b) => b.rawDate - a.rawDate);
      setReports(fetchedReports);
      setMetrics({ totalIncome: tIncome, totalExpenses: tExpense, totalProfit: tIncome - tExpense });

      // Generate Chart Data (Last 6 Months)
      const cMonths = [], cIncome = [0,0,0,0,0,0], cExpense = [0,0,0,0,0,0], cProfit = [0,0,0,0,0,0];
      for (let i = 5; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        cMonths.push(d.toLocaleString('default', { month: 'short' }));
      }
      ...
      localInvoices.forEach(inv => { const i = getMonthIdx(inv.date || inv.createdAt); if(i !== -1) cIncome[i] += Number(inv.total_amount || inv.total || 0); });
      localExpenses.forEach(exp => { const i = getMonthIdx(exp.date || exp.createdAt); if(i !== -1) cExpense[i] += Number(exp.amount || 0); });
      for(let i=0; i<6; i++) cProfit[i] = cIncome[i] - cExpense[i];

      setChartData({ income: cIncome, expenses: cExpense, profit: cProfit, months: cMonths });
  ...
  const maxValue = Math.max(...chartData.income, ...chartData.expenses, 1000);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          label="Profit Margin"
          value={`${metrics.totalIncome > 0 ? ((metrics.totalProfit / metrics.totalIncome) * 100).toFixed(1) : 0}%`}
          change="Offline Synced"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-gray-700">
              ...
              {Math.round(chartData.income.reduce((a,b)=>a+b,0) / 6).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="mt-4 p-3 bg-red-50 rounded-lg">
            <p className="text-sm text-gray-700">
              ...
              {Math.round(chartData.expenses.reduce((a,b)=>a+b,0) / 6).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
            {reports.filter(r => r.type === 'Expense').reduce((acc, curr) => {
               const ex = acc.find(a => a.label === curr.category);
               if (ex) ex.amount += curr.amount; else acc.push({ label: curr.category, amount: curr.amount });
               return acc;
            }, []).map((cat, idx) => (
               <DistributionItem key={idx} label={cat.label || 'Other'} amount={cat.amount} percentage={metrics.totalExpenses > 0 ? (cat.amount / metrics.totalExpenses) * 100 : 0} />
            ))}
            {reports.filter(r => r.type === 'Expense').length === 0 && <p className="text-gray-500 text-sm">No expenses recorded for this period.</p>}
          </div>
          <div className="mt-4 pt-4 border-t border-gray-200">
            <p className="text-sm text-gray-700">
              <span className="font-bold">Total Expenses:</span> ₹{metrics.totalExpenses.toLocaleString()}
            </p>
          </div>
        </div>
      </div>
