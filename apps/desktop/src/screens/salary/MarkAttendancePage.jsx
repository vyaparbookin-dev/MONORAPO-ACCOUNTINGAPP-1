...
const SalaryPage = () => {
  ...
  // Navigation: 'home' (Screen 4: Staff List) | 'staff_hub' (Screen 3: Staff Detail) | 'attendance_cal' (Screen 1 & 2: Daily Attendance)
  const [activeScreen, setActiveScreen] = useState("home");
  const [selectedStaffId, setSelectedStaffId] = useState(null);

  // Sub-views inside Staff Hub (Loans / Advances, Salary Slip, Goals)
  const [hubSubView, setHubSubView] = useState("overview"); // 'overview' | 'loans' | 'salary_slips' | 'goals'
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] text-[#1E293B] font-sans antialiased flex justify-center">
      <div className="w-full max-w-lg bg-[#F8FAFC] min-h-screen shadow-2xl flex flex-col relative pb-10">

        {/* ========================================================================= */}
        {/* SCREEN 1: HOME - STAFF LIST (Matches Screenshot 4)                        */}
        {/* ========================================================================= */}
        {activeScreen === "home" && (
          <div className="flex flex-col flex-1">
            ...
            <div className="bg-white px-4 py-3.5 flex items-center justify-between border-b border-slate-100 sticky top-0 z-10 shadow-xs">
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="p-1 text-slate-800 hover:text-blue-600 transition"
                >
            ...
            <div className="p-4 space-y-4 flex-1">
              ...
              {/* Big Blue Add Staff Button */}
              <button
                onClick={handleOpenAddStaff}
                className="w-full py-3.5 bg-[#1D4ED8] hover:bg-[#1E40AF] text-white rounded-xl font-bold text-sm shadow-md flex items-center justify-center gap-2 transition active:scale-[0.99]"
              >
              ...
              <div className="space-y-4 pt-2">
                ...
                {/* Promo Card at Bottom (Like Screenshot 4) */}
        ...
        {/* ========================================================================= */}
        {/* SCREEN 2: STAFF HUB (Matches Screenshot 3)                                */}
        {/* ========================================================================= */}
        {activeScreen === "staff_hub" && currentStaff && (
          <div className="flex flex-col flex-1 animate-fadeIn">
            {/* Top Bar with Name and Edit Button */}
            <div className="bg-white px-4 py-3.5 flex items-center justify-between border-b border-slate-100 sticky top-0 z-10 shadow-xs">
              <div className="flex items-center gap-3">
                <button 
              ...
              <button 
                ...
                className="text-blue-600 font-bold text-sm hover:underline px-2 py-1 rounded-md active:bg-blue-50"
              >
                Edit
              </button>
            </div>
            ...
            <div className="p-4 space-y-4 flex-1">
              ...
              <div className="pt-2">
                <div className="flex items-center justify-between mb-3 px-1">
                  ...
                  <button 
                    onClick={() => handleOpenEditStaff(currentStaff)}
                    className="flex items-center gap-1 text-blue-600 font-bold text-xs border border-blue-200 px-3 py-1.5 rounded-xl bg-white hover:bg-blue-50"
                  >
                    <span>••• Actions</span>
                  </button>
                </div>
                ...
                <div className="bg-white rounded-2xl p-2.5 border border-slate-100 flex items-center justify-between mb-3">
                  <button onClick={() => handleMonthChange(-1)} className="p-1 hover:bg-slate-100 rounded-lg">
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-xs space-y-4">
                  ...
                  {/* Pay Salary & WhatsApp Actions */}
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    <button
                      onClick={() => handleShareWhatsApp(currentStaff)}
                      className="py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition"
                    >
                    ...
                    <button
                      onClick={() => setShowAdvanceModal(true)}
                      className="py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition"
                    >
                      ...
                      {currentStaff.advancesList.map((adv, idx) => (
                        <div key={adv._id || idx} className="flex items-center justify-between text-xs py-1.5 border-b border-slate-50 last:border-0">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {adv._id && (
                              <button onClick={() => handleDeleteAdvance(adv._id)} className="text-slate-300 hover:text-rose-500 p-1">
        ...
        {/* ========================================================================= */}
        {/* SCREEN 3: ATTENDANCE CALENDAR (Matches Screenshots 1 & 2)                  */}
        {/* ========================================================================= */}
        {activeScreen === "attendance_cal" && currentStaff && (
          <div className="flex flex-col flex-1 animate-fadeIn">
            {/* Top Bar with History, Settings & Help */}
            ...
            <div className="p-4 space-y-4 flex-1">
              ...
              {/* Summary 3x3 Metric Grid (Exact like Screenshot 2) */}
              ...
              {/* Manage Leaves Tile (Screenshot 2) */}
              <div 
                onClick={() => handleOpenEditStaff(currentStaff)}
              ...
              {/* Day-by-Day List (Descending Order like Screenshots 1 & 2) */}
                ...
                {Array.from({ length: daysCount }, (_, i) => daysCount - i).map((dayNum) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 3 Rounded Pill Buttons + Dropdown (P, HD, AB) */}
                      <div className="flex items-center gap-2">
                        {/* Button 1: P (Present) */}
                        <button
                          onClick={() => handleMarkDayAttendance(currentStaff._id, dayNum, "present")}
                          className={`flex-1 py-2 rounded-xl text-xs font-bold transition flex items-center justify-center ${
                            status === "present"
                        ...
                        <button 
                          ...
                          className="p-2 bg-[#F1F5F9] text-blue-600 rounded-xl hover:bg-slate-200"
                        >
                          <ChevronDown className="w-4 h-4" />
                        </button>
                      </div>
        ...
        {/* ========================================================================= */}
        {/* MODAL 1: ADD / EDIT STAFF & SALARY WITH DUAL BOXES & DELETE BUTTON         */}
        {/* ========================================================================= */}
        {showStaffModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="bg-white rounded-3xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                ...
                <button onClick={() => setShowStaffModal(false)} className="p-1 text-slate-400 hover:text-slate-700">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleSaveStaffModal} className="space-y-4 text-xs font-bold text-slate-700">
                ...
                <div className="space-y-2 pt-3 border-t border-slate-100">
                  <button
                    ...
                    className="w-full py-3 bg-[#1D4ED8] hover:bg-[#1E40AF] text-white rounded-xl font-bold text-sm shadow-md transition disabled:opacity-50"
                  >
                  ...
                  {isEditingStaff && modalStaffId && (
                    <button
                      ...
                      className="w-full py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl font-bold text-xs transition flex items-center justify-center gap-1.5 border border-rose-200"
                    >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl border border-slate-100">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                ...
                <button onClick={() => setShowAdvanceModal(false)} className="p-1 text-slate-400 hover:text-slate-700">
                  <X className="w-5 h-5" />
                </button>
              </div>
